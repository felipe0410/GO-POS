...
const formatExpirationDate = () => {
  ...
  return `${year}-12-31`; // Retornar el último día del año en formato YYYY-MM-DD
};

export const transformToDianInvoice = (data: any, establishment: any): any => {
  // Mapeo de los datos de la factura actual
  return {
    resolution_number: establishment.Resolucion, // Resolución fija
    prefix: establishment.Prefijo, // Prefijo fijo
    document_number: parseInt(data.invoice, 10), // Número de la factura actual
    ...
    lines: data.compra.map((item: any) => {
      return {
        ...
        line_extension_amount: item.acc.toFixed(2), // Valor sin impuestos de la línea
        ...
        price_amount: item.acc.toFixed(2), // Precio del artículo
        ...
        tax_totals: [
          {
            ...
            taxable_amount: item.acc.toFixed(2), // Base gravable
    ...
    legal_monetary_totals: {
      line_extension_amount: data.subtotal.toFixed(2), // Total antes de impuestos
      tax_exclusive_amount: data.subtotal.toFixed(2), // Total sin impuestos
      tax_inclusive_amount: data.total.toFixed(2), // Total con impuestos
      payable_amount: data.total.toFixed(2), // Monto total a pagar
    },
  };
};
